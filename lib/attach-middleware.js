...
module.exports = function(app, options) {
  app.post('/write-covfefe',
    ...
    function(req, res) {
      ...
      if (_config.parallel) {
        _config.covfefeFolder = _config.covfefeFolder + '_' + crypto.randomBytes(4).toString('hex');
        if (_config.reporters.indexOf('json') === -1) {
          _config.reporters.push('json');
        }
      }

      var reporter = new Istanbul.Reporter(null, path.join(options.root, _config.covfefeFolder));
      var sync = true;
      ...
      reporter.write(collector, sync, function() {
        var summaryFilePath = path.join(options.root, _config.covfefeFolder, 'coverage-summary.json');
