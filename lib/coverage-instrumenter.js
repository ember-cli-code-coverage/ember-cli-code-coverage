...
CoverageInstrumenter.prototype.processString = async function(content, relativePath) {
  const instrumenter = new EmberInstrumenter({
    embedSource: true,
    noAutoWrap: true
  });

  const realPath = fixPath(relativePath, this._appName, this._appRoot, this._preCompiledExtensions, this._isAddon);
  try {
    return await instrumenter.instrumentSync(content, realPath);
  } catch (e) {
    console.error('Unable to cover:', realPath, '. Please try to enable source maps "inline" on babel conf.\n', e.stack);
  }
